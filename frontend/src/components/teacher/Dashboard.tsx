      ...
      <nav className="nav">
        <div className="nav-inner">
          <div className="min-w-0">
            <h1 className="font-bold text-primary text-sm sm:text-base truncate">Teacher Dashboard</h1>
          ...
          <div className="space-y-5">
            {loading ? (
              ...
              <>
                ...
                <div className="card">
                  <h3 className="font-semibold text-primary mb-4 text-sm sm:text-base">Per-Subject Breakdown</h3>
                  {ratingsData.breakdown.length === 0 ? (
                    <p className="text-secondary text-sm">No feedback received yet.</p>
                  ) : (
                    <div className="table-wrap">
                      <table className="w-full text-sm min-w-[320px]">
                        <thead>
                          <tr className="border-b border-base text-left">
                            <th className="pb-3 text-secondary font-medium">Subject</th>
                            <th className="pb-3 text-secondary font-medium text-right">Avg</th>
                            <th className="pb-3 text-secondary font-medium text-right">Count</th>
                          </tr>
                        </thead>
                          ...
                          {ratingsData.breakdown.map((b: any, i: number) => (
                            <tr key={i} className="hover:bg-gray-500/5 transition-colors">
                              <td className="py-3 text-primary">
                                ...
                                <span className="text-xs text-secondary ml-1 hidden sm:inline">({b.assignment?.subjectId?.code})</span>
                              </td>
                              <td className="py-3 text-right">
                                <span className={`font-bold ${b.averageRating >= 7 ? 'text-emerald-500' : b.averageRating >= 5 ? 'text-yellow-500' : 'text-red-500'}`}>
                                  {b.averageRating}/10
                                </span>
                              </td>
                              <td className="py-3 text-right text-secondary">{b.totalCount}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="card text-center text-secondary text-sm">Failed to load ratings.</div>
            )}
          </div>
        )}
