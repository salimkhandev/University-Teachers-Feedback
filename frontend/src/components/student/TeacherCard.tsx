    ...
    <div className="card hover:border-gray-700 transition-all duration-200">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-white">{teacher?.name ?? 'Unknown Teacher'}</h3>
            <p className="text-sm text-gray-400">{subject?.name ?? 'Unknown Subject'}
              {subject?.code && <span className="ml-2 text-xs text-gray-600">({subject.code})</span>}
            </p>
          </div>
        </div>
      ...
      {assignment.feedbackSubmitted && (
        <div className="mt-3 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400">{assignment.currentRating}/10</span>
        </div>
      )}
      ...
      {open && (
        <div className="mt-4 pt-4 border-t border-gray-800">
          <FeedbackForm assignment={assignment} onDone={() => { setOpen(false); onRefresh(); }} />
        </div>
      )}
    </div>
  );
